...
export default function ShaContributions() {
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(
            `https://b6d41abe4044.ngrok-free.app/api/sha-contributions/${id}`,
            {
              method: "DELETE",
              headers: { "ngrok-skip-browser-warning": "true" },
            }
  ...
  return (
    <div className="bg-white px-4 py-4 rounded-lg relative">
      ...
      {/* Drawers */}
      <AddContributionDrawer open={openAdd} onClose={() => setOpenAdd(false)} onSuccess={fetchContributions} />
      <EditContributionDrawer open={openEdit} onClose={() => setOpenEdit(false)} onSuccess={fetchContributions} contribution={selectedContribution} />
    </div>
  );
}
